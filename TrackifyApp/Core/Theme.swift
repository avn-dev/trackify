...
struct Theme {
    ...
    static let dark = Theme(
        ...
        accent: Palette.accentLime, accentText: Palette.accentText,
...
struct ThemedRoot<Content: View>: View {
    ...
    private var resolvedTheme: Theme {
        let option = AccentOption(rawValue: accentKey) ?? .lime
        var theme  = scheme == .dark ? Theme.dark : Theme.light
        theme.accent     = option.color
        theme.accentText = option.textColor
        return theme
    }
